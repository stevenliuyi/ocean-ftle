...
# map domain
nx = 1200; ny = 600
imin = 200; imax = 300; mx = imax - imin + 1
jmin = 100; jmax = 200; my = jmax - jmin + 1
...
trajx = []; trajy = []
for k in range(0, 20):
    (trajxk, nx, ny) = read('traj_x_pos_' + t_str \
            + '_' + str(k) + '.txt')
    (trajyk, nx, ny) = read('traj_y_pos_' + t_str \
            + '_' + str(k) + '.txt')
...
m.drawparallels(np.linspace(m_down_lat,m_up_lat,4),labels=[1,0,0,0], \
        fmt='%.1f')
m.drawmeridians(np.linspace(m_left_lon,m_right_lon,4),labels=[0,0,0,1], \
        fmt='%.1f')
...
plt.title("FTLE, " + plot_type + "and trajectories [Day " + str(t) + "]")
fig.savefig('ftle_traj_' + t_str + '.png')
